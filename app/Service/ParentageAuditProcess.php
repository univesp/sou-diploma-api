...
class ParentageAuditProcess
{
    ...
    public function __construct($request, $students, $parentages)
    {
    ...
    public function checkBefore()
    {
        foreach ($this->request as $key => $r) {
            foreach ($this->parentages->toArray() as $keyParentages => $p) {
                if ($keyParentages === $key) {
                    foreach ([$keyParentages => $p] as $value) {
                        return $value;
                    }
                }
            }
